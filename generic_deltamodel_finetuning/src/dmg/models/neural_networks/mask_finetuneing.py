...
from models.neural_networks.transformer.StefaLand_PatchTokens_TFT_old import Model as StefaLandPatchTFTold
...
class MaskFinetuneing(nn.Module):
    ...
    def _initialize_pretrained_model(self):
        ...
        if pretrained_type == 'mfformer':
        ...
        elif pretrained_type in ['stefaland_patch_tft', 'stefaland_tft_patches']:
            # StefaLand with Patch tokens + TFT conditioning
            cfg = type('Config', (), {
                'd_model': self.model_config['d_model'],
                'num_heads': self.model_config['num_heads'],
                'dropout': self.model_config['dropout'],
                ...
                'warmup_train': False,
                'add_input_noise': False,
                # Patch-TST specifics (defaults OK if not in config)
                'use_patches': True,
                'patch_len': getattr(self, 'patch_len', 16),
                'patch_stride': getattr(self, 'patch_stride', 8),
                # Optional grouped static tokens; pass through if provided
                'group_mask_dict': getattr(self, 'group_mask_dict', {}),
            })
            built_model = StefaLandPatchTFT(cfg).float()
            return self._load_pretrained_weights(built_model)

        elif pretrained_type in ['stefaland_patch_tft_old', 'stefaland_tft_patches_old']:
            cfg = type('Config', (), {
                # same fields as above; tweak if the old variant needs different defaults
                'd_model': self.model_config['d_model'],
                'num_heads': self.model_config['num_heads'],
                'dropout': self.model_config['dropout'],
                'num_enc_layers': self.model_config['num_enc_layers'],
            ...
            built_model = StefaLandPatchTFTold(cfg).float()
            return self._load_pretrained_weights(built_model)
    ...
    def _load_pretrained_weights(self, model):
        ...
        If no valid path is given or loading fails, we fall back to the
        randomly initialized `model` of the correct pretrained_type.
        """
        pretrained_model_path = self.model_config['pretrained_model']

        # Case 1: no path given -> just use random init
        if not pretrained_model_path:
            log.warning(
                f"No pretrained model path provided; using randomly initialized "
                f"{self.model_config['pretrained_type']} model"
            )
            # Make sure it is trainable
        ...
        try:
            ...
            if os.path.isdir(pretrained_model_path):
                checkpoint_list = [
                    f for f in os.listdir(pretrained_model_path)
            ...
            # Only freeze if we actually loaded something
            ...
            # Case 2: path provided but load fails -> fall back to random init
            log.error(
                f"Error loading pretrained model from '{pretrained_model_path}': {e}. "
                f"Falling back to randomly initialized {self.model_config['pretrained_type']} model."
            )
            for p in model.parameters():
                p.requires_grad = True
            return model
    ...
    def _encode_with_stefaland_masked(self, batch_x_ft, batch_c_ft, temporal_features=None):
        # Map fine-tune inputs to pretrained layout with masks
        batch_x_pre = prepare_masked_input(batch_x_ft, self.ts_mapping, self.ts_missing_mask, len(self.pretrained_ts_vars))
        batch_c_pre = prepare_masked_input(batch_c_ft, self.static_mapping, self.static_missing_mask, len(self.pretrained_static_vars))

        time_series_mask = torch.tensor(self.ts_missing_mask, device=batch_x_pre.device)\
                            .unsqueeze(0).unsqueeze(0).expand_as(batch_x_pre)
        static_mask = torch.tensor(self.static_missing_mask, device=batch_c_pre.device)\
                        .unsqueeze(0).expand_as(batch_c_pre)
        ...
        if temporal_features is not None:
            bd['temporal_features'] = temporal_features

        with torch.amp.autocast('cuda', enabled=False):
            od = self.pretrained_model(bd, is_mask=False)

        if not hasattr(self, "_stefaland_proj"):
            inF = od['outputs_time_series'].size(-1)
            self._stefaland_proj = nn.Linear(inF, self.model_config['d_model']).to(od['outputs_time_series'].device)

        emb = self._stefaland_proj(od['outputs_time_series'])  # [B, T, d_model]
        return emb
    ...
    def forward(self, xc_nn_norm, temporal_features=None, station_ids=None) -> torch.Tensor:
        ...
        elif self.model_config['pretrained_type'] in ['stefaland_patch_tft','stefaland_tft_patches','stefaland_patch_tft_old','stefaland_tft_patches_old']:
            return self._forward_with_stefaland(xc_nn_norm, temporal_features)
    ...
    def _forward_with_stefaland(self, xc_nn_norm, temporal_features=None):
        # Split fine-tune input into time series and static components
        n_ts_vars = len(self.finetuning_ts_vars)
        n_static_vars = len(self.finetuning_static_vars)

        # Fine-tune inputs (your adapter & decoder want the FT variables, not the masked layout)
        batch_x_ft = xc_nn_norm[..., :n_ts_vars]                                   # [*, T, n_ts_vars]
        batch_c_ft = xc_nn_norm[..., n_ts_vars:n_ts_vars+n_static_vars][..., 0, :] # [*, n_static_vars]

        # Encode with StefaLand using PRETRAINED variable layout + masks
        hidden_states = self._encode_with_stefaland_masked(batch_x_ft, batch_c_ft, temporal_features)  # [B, T, d_model]

        # Normalize + scale (same as others)
        hidden_states = self.embedding_norm(hidden_states) * self.embedding_scale

        # Adapter with FULL fine-tune inputs
        adapted = self._apply_adapter(hidden_states, batch_x_ft, batch_c_ft)
        ...
        # Final projection to ny
        outputs = self.projection(outputs)  # torch.Tensor
        return outputs
