...
class Finetuneing(nn.Module):
    ...
    def _initialize_pretrained_model(self):
        ...
        if pretrained_type == 'mfformer':
            ...
            built_model = StefaLandPatchTFT_Old(cfg).float()
            return self._load_pretrained_weights(built_model)
    ...
    def _encode_with_stefaland(self, batch_x, batch_c, temporal_features=None):
        # Build the dict StefaLand expects; no extra masking during fine-tune inference
        bd = {
            'batch_x': batch_x,                    # [B, T, F]
            'batch_c': batch_c,                    # [B, C]
        ...
        if temporal_features is not None:
            bd['temporal_features'] = temporal_features  # [B, T, K]

        with torch.amp.autocast('cuda', enabled=False):
            od = self.pretrained_model(bd, is_mask=False)

        # od['outputs_time_series'] is [B, T, F]; project to [B, T, d_model]
    ...
    def _forward_with_mfformer(self, xc_nn_norm, temporal_features=None, station_ids=None):
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            if self.model_config['pretrained_type'] in ['stefaland_patch_tft', 'stefaland_tft_patches',
                                                    'stefaland_patch_tft_old', 'stefaland_tft_patches_old']:
                n_time_series = len(self.model_config['time_series_variables'])
                batch_x = xc_nn_norm[..., :n_time_series]
                batch_c = xc_nn_norm[..., n_time_series:][..., 0, :]

                # NaN handling (same as other branches)
                x_mask = torch.isnan(batch_x); c_mask = torch.isnan(batch_c)
                x_median = torch.nanmedian(batch_x) if not torch.isnan(batch_x).all() else torch.tensor(0.0, device=batch_x.device)
                c_median = torch.nanmedian(batch_c) if not torch.isnan(batch_c).all() else torch.tensor(0.0, device=batch_c.device)
                batch_x = batch_x.masked_fill(x_mask, x_median)
                batch_c = batch_c.masked_fill(c_mask, c_median)

                hidden_states = self._encode_with_stefaland(batch_x, batch_c, temporal_features)
                hidden_states = self.embedding_norm(hidden_states) * self.embedding_scale

                # proceed exactly like the others
                adapted = self._apply_adapter(hidden_states, batch_x, batch_c)
                if self.model_config['use_residual_lstm']:
                    outputs = self._decode_with_residual_lstm(adapted, batch_x, batch_c)
                else:
