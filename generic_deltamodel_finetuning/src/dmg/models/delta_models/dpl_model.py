...
class DplModel(torch.nn.Module):
    ...
    def forward(self, data_dict: dict[str, torch.Tensor], eval: bool = False) -> torch.Tensor:
        current_forecast_days = data_dict.get('current_forecast_days', 1)

        nn_name = type(self.nn_model).__name__

        # Only switch to dict pass-through when extra pretrained inputs exist.
        has_pretrained = ('xc_pretrained_norm' in data_dict) and (data_dict['xc_pretrained_norm'] is not None)

        # Helper: does nn forward accept current_forecast_days kwarg?
        nn_forward = getattr(self.nn_model, "forward", None)
        nn_accepts_cfd = False
        if nn_forward is not None and hasattr(nn_forward, "__code__"):
            nn_accepts_cfd = ('current_forecast_days' in nn_forward.__code__.co_varnames)

        # --- NEW: dict pass-through path (only when needed, only for the right NN)
        # MaskFinetuneing expects the batch dict if we want it to see xc_pretrained_norm and temporal_features.
        if has_pretrained and nn_name == 'MaskFinetuneing':
            if nn_accepts_cfd:
                parameters = self.nn_model(data_dict, current_forecast_days=current_forecast_days)
            else:
