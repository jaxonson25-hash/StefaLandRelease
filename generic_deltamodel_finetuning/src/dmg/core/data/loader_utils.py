...
def split_by_basin(dataset: Dict[str, torch.Tensor], config: Dict[str, Any], 
    ...
    try:
        ...
        log.info(f"[split_by_basin] basin_file_path={basin_file_path}")
        log.info(f"[split_by_basin] dataset keys={list(dataset.keys())} x_nn shape={dataset.get('x_nn', None).shape if dataset.get('x_nn', None) is not None else None}")
        ...
        for key, tensor in dataset.items():
            ...
            n_basins = None
            if dataset.get("c_nn") is not None:
                n_basins = int(dataset["c_nn"].shape[0])
            else:
                n_basins = len(all_basins)
            log.info(f"[split_by_basin] key={key} shape={tuple(cpu_tensor.shape)} n_basins={n_basins}")

            if len(cpu_tensor.shape) == 3:
                if cpu_tensor.shape[0] == n_basins:
                    train_data[key] = cpu_tensor[train_indices_tensor]
                    test_data[key]  = cpu_tensor[test_indices_tensor]
                elif cpu_tensor.shape[1] == n_basins:
                    train_data[key] = cpu_tensor[:, train_indices_tensor]
                    test_data[key]  = cpu_tensor[:, test_indices_tensor]
                else:
                    train_data[key] = cpu_tensor
                    test_data[key]  = cpu_tensor

            elif len(cpu_tensor.shape) == 2:
                # Only split if this is actually [basins, features]
                if cpu_tensor.shape[0] == n_basins:
                    train_data[key] = cpu_tensor[train_indices_tensor]
                    test_data[key]  = cpu_tensor[test_indices_tensor]
                else:
                    # e.g., temporal_features = [time, feat] should not be split by basin
                    train_data[key] = cpu_tensor
                    test_data[key]  = cpu_tensor
...
def normalize_data_split(
    ...
    """
    Split normalization WITHOUT repeating statics over time.

    Inputs
      x_nn: [time, basins, n_forcings]
      c_nn: [basins, n_static]

    Returns
      x_nn_norm: [time, basins, n_forcings]
      c_nn_norm: [basins, n_static]
    """

    # Match existing normalize_data() behavior but stop before repeat/concat
    x_nn_norm = to_norm(
        np.swapaxes(x_nn, 1, 0).copy(),   # [time, basins, nf] -> [basins, time, nf]
        nn_forcings,
        norm_stats,
        log_norm_vars
    )  # to_norm swaps back -> [time, basins, nf] :contentReference[oaicite:1]{index=1}
    ...
    # Remove nans (keep same convention as normalize_data) :contentReference[oaicite:2]{index=2}
    x_nn_norm[x_nn_norm != x_nn_norm] = 0
    c_nn_norm[c_nn_norm != c_nn_norm] = 0

    return x_nn_norm.astype(np.float32, copy=False), c_nn_norm.astype(np.float32, copy=False)
