...
def _aggregate_spatial_results(predictions: List, targets: List, 
    ...
    try:
        ...
        for i, (pred, target) in enumerate(zip(predictions, targets)):
            ...
            # ---- normalize orientation: make both [time, basins] ----
            if pred_array.ndim == 2 and target_array.ndim == 2:
                # target is often [time, basins]. If pred is [basins, time], transpose it.
                if pred_array.shape == target_array.T.shape:
                    log.info(f"Holdout {i}: Transposing predictions to match target orientation")
                    pred_array = pred_array.T

                # If target is [basins, time] and pred is [time, basins], transpose target
                elif target_array.shape == pred_array.T.shape:
                    log.info(f"Holdout {i}: Transposing targets to match prediction orientation")
                    target_array = target_array.T

                # After this, they should match on time dimension
                if pred_array.shape[0] != target_array.shape[0]:
                    raise ValueError(
                        f"Holdout {i}: time dimension mismatch after orientation fix: "
